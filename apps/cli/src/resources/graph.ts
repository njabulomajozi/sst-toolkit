...
import type { IResource } from "./types.js";
...
/**
 * Group resources by deletion priority
 * Some resources should be deleted in specific order regardless of dependencies
 * Returns groups in deletion order (first group deleted first)
 */
export function groupByDeletionPriority(resources: IResource[]): IResource[][] {
  const groups: IResource[][] = [[], [], [], [], []]; // 5 priority levels
  ...
  for (const resource of resources) {
    ...
    switch (resource.service) {
      case "events":
        // EventBridge rules should be deleted early (after removing targets)
        priority = 1;
        break;

      case "lambda":
        if (resource.resourceType === "event-source-mapping") {
          // Event source mappings should be deleted before functions
          priority = 0;
        } else {
          // Functions should be deleted after their event source mappings
          priority = 2;
        }
        break;

      case "logs":
        // Log groups should be deleted after their associated functions
        priority = 3;
        break;

      case "apigateway":
        // API Gateway should be deleted early
        priority = 1;
        break;

      case "ec2":
        // Route tables should be deleted after disassociating
        priority = 2;
        break;

      case "s3":
        // S3 buckets can be deleted after emptying
        priority = 2;
        break;

      case "dynamodb":
        // DynamoDB tables can be deleted anytime
        priority = 2;
        break;

      case "sqs":
        // SQS queues can be deleted anytime
        priority = 2;
        break;

      case "servicediscovery":
        // Service Discovery namespaces can be deleted anytime
        priority = 2;
        break;

      default:
        priority = 4; // Unknown resources last
    }
 ...
 * 1. EventBridge rules (remove targets first, then rule)
 * 2. Lambda event source mappings (before functions)
 * 3. API Gateway APIs
 * 4. Lambda functions (after event source mappings)
 * 5. CloudWatch Log Groups (after functions)
 * 6. Storage resources (S3, DynamoDB, SQS)
 * 7. Networking resources (EC2, Service Discovery)
 */
