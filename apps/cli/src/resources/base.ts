...
import type { IResource, IFinderOptions } from "./types.js";
  ...
  do {
    ...
    if (response.ResourceTagMappingList) {
      for (const resource of response.ResourceTagMappingList) {
        ...
        // Optimize tag verification with Map (O(1) lookup instead of O(n))
        ...
        if (matches) {
          ...
          if (parsed) {
            ...
            if (filterService && parsed.service !== filterService) continue;
            if (filterResourceType && parsed.resourceType !== filterResourceType) continue;
