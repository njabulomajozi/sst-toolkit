...
export interface IPluginCreateOptions {
  name: string;
...
export async function createPlugin(options: IPluginCreateOptions): Promise<void> {
  const { name, template, namespace, outputDir = process.cwd() } = options;
